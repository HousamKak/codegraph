...
We have implemented the complete Software Physics framework as \textbf{CodeGraph}, an open-source system available at GitHub. The implementation consists of three main components deployed in production environments.

\subsubsection{Backend Architecture}

The backend, built with FastAPI, provides two interfaces for code analysis:

\textbf{REST API}: 15+ endpoints covering indexing, validation, snapshots, git integration, and real-time updates via WebSockets. The API serves as the foundation for web-based frontends and integrations.

\textbf{MCP Server} (Model Context Protocol): 13 tools enabling direct LLM integration:
\begin{itemize}
    \item \textit{Indexing}: index\_codebase, get\_graph\_stats
    \item \textit{Querying}: find\_function, get\_function\_details, get\_function\_callers, get\_function\_callees, get\_function\_dependencies, search\_code
    \item \textit{Analysis}: analyze\_impact, validate\_codebase
    \item \textit{Snapshots}: create\_snapshot, compare\_snapshots, list\_snapshots
\end{itemize}

The MCP server enables LLMs like Claude to use CodeGraph as a read-only analysis tool while performing all code editing through their standard file manipulation tools.
...
\begin{itemize}
    \item \textbf{Graph Database}: Neo4j 5.15 Community Edition with Cypher queries, APOC procedures, and custom indexes
    \item \textbf{Parser}: Python's built-in AST module for syntax tree traversal
    \item \textbf{Type Checker}: pyright integration for deep type analysis
    \item \textbf{Backend}: FastAPI 0.104+ with async/await, Pydantic models, uvicorn ASGI server
    \item \textbf{Frontend}: React 18 with TypeScript, Cytoscape.js for visualization, Zustand for state management
    \item \textbf{Git Integration}: GitPython for commit tracking and diff generation
    \item \textbf{Real-time}: WebSocket support for live graph updates
    \item \textbf{Deployment}: Docker Compose for development, Kubernetes-ready for production
\end{itemize}

\subsubsection{Graph Schema Evolution}

The implemented schema underwent optimization based on theoretical insights:

\textbf{Schema Refinement}: We eliminated redundant relationships (CONTAINS merged into DECLARES, CALLS merged into RESOLVES\_TO) reducing edge count by 30-40\% while maintaining full semantic expressiveness.

\textbf{Current Schema} includes 7 node types (Function, Class, Module, Parameter, Variable, CallSite, Type, Decorator) and 14 relationship types. The RESOLVES\_TO relationship replaces the original CALLS by tracking both the call site and resolution status, enabling precise R law validation.
...
We present both theoretical analysis and empirical results from the production implementation.
...
We evaluated CodeGraph on real Python codebases of varying sizes:

\begin{table}[h]
\centering
\begin{tabular}{lrrrr}
\toprule
\textbf{Project} & \textbf{Files} & \textbf{Functions} & \textbf{Nodes} & \textbf{Index Time} \\
\midrule
Example Project & 3 & 14 & 42 & 0.5s \\
CodeGraph Backend & 15 & 120 & 450 & 3.2s \\
Medium Codebase & 50 & 500 & 1800 & 12.5s \\
\bottomrule
\end{tabular}
\caption{Indexing performance on real Python codebases}
\label{tab:performance}
\end{table}

Validation performance scales with changed nodes:
\begin{itemize}
    \item \textbf{Full validation}: 1-3 seconds for 500-function codebase
    \item \textbf{Incremental} (10 changed nodes): 0.1-0.3 seconds
    \item \textbf{Single file}: 0.05-0.15 seconds
\end{itemize}

\subsection{Validation Accuracy}

Testing on codebases with known issues:

\begin{table}[h]
\centering
\begin{tabular}{lrrr}
\toprule
\textbf{Violation Type} & \textbf{Injected} & \textbf{Detected} & \textbf{Rate} \\
\midrule
Arity Mismatch & 25 & 25 & 100\% \\
Unresolved Calls & 18 & 18 & 100\% \\
Missing Parameters & 12 & 11 & 92\% \\
Inheritance Cycles & 5 & 5 & 100\% \\
\midrule
\textbf{Total} & \textbf{60} & \textbf{59} & \textbf{98.3\%} \\
\bottomrule
\end{tabular}
\caption{Violation detection accuracy}
\label{tab:accuracy}
\end{table}

The single missed violation was a parameter with a complex default value that required static analysis beyond AST parsing.

\subsection{Case Study: LLM-Driven Refactoring}

We tested the framework with Claude using the MCP server to refactor a 200-function Python project:

\textbf{Task}: Rename a widely-used utility function and update all 15 call sites.

\textbf{Without CodeGraph}:
\begin{itemize}
    \item LLM updated function definition
    \item Missed 3 of 15 call sites (20\% miss rate)
    \item Errors discovered at runtime
    \item Total time: 2 attempts, 10 minutes
\end{itemize}

\textbf{With CodeGraph}:
\begin{itemize}
    \item LLM used find\_function to locate all callers
    \item Created snapshot before changes
    \item Renamed function and updated all 15 call sites
    \item Validation immediately caught remaining issues
    \item LLM fixed violations in second iteration
    \item Total time: 1 complete workflow, 5 minutes
    \item Final validation: 0 violations
\end{itemize}

\subsection{Schema Optimization Impact}

The relationship consolidation (Section 7.1.3) yielded measurable benefits:
...
\midrule
Total Edges & 340 & 215 & 37\% reduction \\
Query Time (callers) & 45ms & 28ms & 38\% faster \\
Validation Time & 1.2s & 0.8s & 33\% faster \\
Storage Size & 2.4MB & 1.6MB & 33\% smaller \\
\bottomrule
\end{tabular}
\caption{Impact of schema optimization on medium codebase}
